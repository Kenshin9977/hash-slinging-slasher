...
Use --kind xmodel or --kind image to measure each naming vocabulary separately.
...
def main():
    ...
    ap.add_argument('--kind', choices=['xanim','xmodel','image'], default='xanim')
    args = ap.parse_args()
    known = {n.strip().lower().replace('\\','/') for n in itertools.chain(
        snapshot.table_names({'xanim':'fnv1a_xanims','xmodel':'fnv1a_xmodels','image':'fnv1a_ximages'}[args.kind]),
